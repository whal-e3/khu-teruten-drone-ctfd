...
INLINE void xor_encrypt(char* data, size_t data_length, unsigned int key) {
    // Perform XOR operation 4 bytes at a time
    for (size_t i = 0; i < data_length; i += 4) {
        // Cast the pointer to an unsigned int pointer for 4-byte operations
        unsigned int* data_ptr = (unsigned int*)(data + i);
        *data_ptr ^= key;
    }
}

const char *enc_mavlink2_packet = "";
void Vehicle::_handleBatteryStatus(mavlink_message_t& message)
{
    ...
    case MAV_BATTERY_CHARGE_STATE_UNHEALTHY:
        ...
        break;
    case MAV_BATTERY_CHARGE_STATE_FLAG:
        if (batteryStatus.charge_state > _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id]) {
            _lowestBatteryChargeStateAnnouncedMap[batteryStatus.id] = batteryStatus.charge_state;
            batteryMessage = tr("battery %1 secret message");
        }

        unsigned int enc_mavlink_packet_length = strlen(enc_mavlink2_packet);
        enc_mavlink_packet_length += enc_mavlink_packet_length % 4;

        char mutable_enc_flag[0x80] = {0, };
        memcpy(mutable_enc_flag, enc_mavlink2_packet, enc_mavlink_packet_length);

        unsigned int key = batteryStatus.fault_bitmask;
        xor_encrypt(mutable_enc_flag, enc_mavlink_packet_length, key);
